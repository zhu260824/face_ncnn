...
MTCNN::MTCNN(const string &model_path) {
    ...
    int pi1 = Pnet.load_param(param_files[0].data());
    int bi1 = Pnet.load_model(bin_files[0].data());
    int pi2 = Rnet.load_param(param_files[1].data());
    int bi2 = Rnet.load_model(bin_files[1].data());
    int pi3 = Onet.load_param(param_files[2].data());
    int bi3 = Onet.load_model(bin_files[2].data());
}
...
void MTCNN::detect(cv::Mat &img_, std::vector<Bbox> &finalBbox_) {
    ncnn::Mat ncnn_img = ncnn::Mat::from_pixels(img_.data, ncnn::Mat::PIXEL_BGR2RGB, img_.cols,
                                                img_.rows);
    img = ncnn_img;
    img_w = img.w;
    img_h = img.h;
